> [!WARNING]
> **安全警示 —— 本项目已停止维护，现仅作为安全研究资料存档**
>
> **原上游仓库自初始提交起即被植入供应链后门**：`includes/vendor/composer/platform_check.php` 与 `autoload_real.php` 中，攻击者只需在 HTTP 请求中添加 `PHP-Version: 70200` 请求头，站点即通过 `Set-Cookie` 响应头把系统核心密钥 `syskey` 泄露给攻击者，可用于伪造管理/商户登录态与支付回调。
>
> 本仓库虽已在提交 `e9fff64` 中清除上述两处植入，但整个代码库均源自该带毒上游，**无法确保不存在其他未被发现的后门或恶意代码**。出于安全考虑，本仓库已**停止维护**并转为存档，**现仅供安全研究与学习用途，严禁用于生产环境或任何涉及真实资金的场景**。
>
> 同时严肃提醒：**该上游源码存在后门**。请警惕任何基于该上游源码进行修改、二次开发的项目——除非经过完整的安全审计，否则同样可能携带此供应链后门。
